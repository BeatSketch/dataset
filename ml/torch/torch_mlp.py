...
try:
    ...
    print(
        colorama.Style.DIM
        + colorama.Fore.RED
        + "PyTorch installation not found."
    )
    print(
        colorama.Fore.RESET
        + "To fix, install PyTorch. See README.md for instructions."
    )
    exit(1)
...
def train_model(dataset: DATASET_TYPE) -> str:
    ...
    for epoch in range(epochs):
        ...
        for batch in tqdm(train_loader, total=len(train_loader), desc=f"Epoch {epoch+1}/{epochs}", leave=True):
    ...
    with torch.no_grad():
        ...
        for batch in tqdm(test_loader, total=len(test_loader), desc=f"Evaluation", leave=True):
            batch = {k: v.to(device) for k, v in batch.items()}
